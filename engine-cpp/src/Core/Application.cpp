...
Application::Application(int width, int height, const std::string& title) {
    ...
    SetExitKey(KEY_NULL); // para que ESC abra el menú de pausa en vez de cerrar la ventana
...
Application::~Application() {
    // Los recursos de GPU/audio se liberan a mano, antes de cerrar la ventana y el audio.
    m_level = LevelData{};
    m_toonShader.reset();
    m_music.reset();
...
void Application::LoadLevel(const std::string& path) {
    ...
    // Limpia el estado de la partida anterior antes de cargar la nueva.
    m_spawners.clear();
    m_projectiles.clear();
    m_puddles.clear();
...
void Application::AdvanceToNextStoryLevel() {
    ...
    // Se acabó la historia: vuelve al selector de niveles.
...
void Application::LaunchLevelEditor() const {
    ...
    // Pasa el idioma actual para que el editor abra ya en el mismo idioma.
    std::string command = std::string("start \"\" \"") + kEditorPath + "\" " + m_localization.GetCurrentLanguage();
    std::system(command.c_str());
}

void Application::UpdateMenu() {
    if (IsKeyPressed(KEY_F12)) { // atajo rápido para abrir el editor de niveles
        LaunchLevelEditor();
    }
    ...
    // Mientras Opciones está abierta, aplica el volumen en vivo y lo guarda al soltar el slider.
    ...
    switch (action) {
        ...
        case MenuAction::BackToMainMenu:
            // Opciones y Guía vuelven a donde se abrieron; el resto siempre vuelve al menú principal.
...
void Application::ResolveBarrelExplosions() {
    // Repite hasta que no quede ningún barril por resolver, para que una explosión
    // pueda encadenar la de otro barril cercano.
...
void Application::UpdateActiveMatch(float dt) {
    ...
    // Con el hit-stop activo, las entidades reciben dt = 0 y se congelan un instante.
    ...
    if (!meleeHits.empty()) {
        ...
        for (const MeleeHitResult& hit : meleeHits) {
            // Menos chispas si el golpe fue bloqueado por un Shielder.
    ...
    // Elimina a los enemigos que ya terminaron de desvanecerse.
...
void Application::DrawGroundGrid() const {
    // Cuadrícula holográfica azul que cubre toda la arena del nivel.
...
void Application::DrawGameplay() const {
    ...
    // Los charcos se dibujan antes que el resto, como una capa del suelo.
    for (const MudPuddle& puddle : m_puddles) {
        float fade = puddle.lifetime / MudPuddle::kLifetime; // se apaga según se va secando
...
void Application::TriggerHitStop(float duration) {
    m_hitStopTimer.Start(std::max(m_hitStopTimer.Remaining(), duration)); // no acorta un hit-stop ya activo
}
